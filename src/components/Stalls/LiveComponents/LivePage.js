...
import { generateAgoraToken, getChannelName } from '../../../services/agoraTokenService.js'
...
export default {
  ...
  data() {
    return {
      ...
      // Agora Live Streaming Properties
  ...
  methods: {
    ...
    async startLive() {
      try {
        this.loading = true
        await this.initializeAgoraStream()
        this.handleMessage('Live streaming started successfully!', 'success')
      } catch (error) {
        console.error('Error starting live:', error)
        this.handleMessage('Failed to start live streaming', 'error')
      } finally {
        this.loading = false
      }
    },
    async stopLive() {
      try {
        this.loading = true
        await this.stopAgoraStream()
        this.handleMessage('Live streaming stopped', 'info')
      } catch (error) {
        console.error('Error stopping live:', error)
        this.handleMessage('Failed to stop live streaming', 'error')
      } finally {
        this.loading = false
      }
    },
    ...
    // Agora Live Streaming Methods
    async initializeAgoraStream() {
      try {
        // Generate channel name based on stall ID and type
        this.agoraChannelName = getChannelName(this.stallId, this.type)

        // Get Agora token (for now returns null, works in test mode)
        const token = await generateAgoraToken(this.agoraChannelName, 0, 'publisher')

        // Join Agora channel as broadcaster
        this.agoraUid = await agoraService.joinChannel(this.agoraChannelName, token)

        // Get video element for local preview
        await this.$nextTick()
        ...
        await agoraService.createAndPublishTracks(videoElement)

        this.isLiveActive = true
        this.agoraConnected = true

        console.log('✅ Agora streaming started:', {
          channel: this.agoraChannelName,
          uid: this.agoraUid,
        })

        // Save stream info to share with mobile (you can store this in backend)
        this.saveStreamInfo()
      } catch (error) {
    ...
    async stopAgoraStream() {
      try {
        await agoraService.leaveChannel()
        this.isLiveActive = false
        this.agoraConnected = false
        this.agoraChannelName = null
        this.agoraUid = null

        console.log('✅ Agora streaming stopped')
      } catch (error) {
        console.error('❌ Failed to stop Agora stream:', error)
        throw error
      }
    },

    saveStreamInfo() {
      // Store stream info that mobile app can access
      const streamInfo = {
        channelName: this.agoraChannelName,
        stallId: this.stallId,
        type: this.type,
        startTime: new Date().toISOString(),
        broadcasterUid: this.agoraUid,
      }

      // TODO: Send this to backend so mobile app can fetch it
      console.log('📡 Stream Info:', streamInfo)

      // For now, store in sessionStorage (mobile would call API to get this)
      sessionStorage.setItem('currentStream', JSON.stringify(streamInfo))
    },
