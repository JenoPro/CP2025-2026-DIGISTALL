    ...
    return {
      ...
      rules: {
        ...
        size: [
          ...
          (v) =>
            /^\d+x\d+/i.test(v) || 'Size format should be like "3x3" or "3x3 meters"',
        ],
  ...
  methods: {
    ...
    async handleSave() {
      ...
      try {
        ...
        console.log(
          "🔗 Making API call to:",
          `${this.apiBaseUrl}/api/stalls/${this.editForm.id}`
        );
        ...
        if (!response.ok) {
          if (response.status === 401) {
            ...
            throw new Error(
              "Access denied - you do not have permission to update this stall"
            );
